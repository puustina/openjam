...
function venus.registerEvents()
	...
	for _, f in ipairs(all_callbacks) do
		...
		love[f] = function(...)
			...
			-- fire the original love.x -function
			registry[f](...)
	  
			-- if the current state has that function, we will fire it
			if venus.current[f] then
				return venus.current[f](venus.current, ...)
			end
...
function venus._switch(to, ...)
	...
	if pre.leaving then pre:leaving() end
	...
	if to.entered then to:entered() end
...
function transitions.fade.state:draw()
    if transitions.fade.switched and transitions.fade.to.draw then
		transitions.fade.to:draw()
        --_ = (transitions.fade.to.draw or __NULL__)()
    elseif transitions.fade.pre.draw then
		transitions.fade.pre:draw()
...
function transitions.fade.state:update(dt)
	if transitions.fade.switched and transitions.fade.to.update then
		transitions.fade.to:update(dt)
	elseif transitions.fade.pre.update then
		transitions.fade.pre:update(dt)
	end
end
...
function transitions.fade.switch(to, duration, ...)
    ...
    local f = function()
		...
		if venus.left.left then venus.left:left() end
		if to.entering then to:entering() end
		switched = true
